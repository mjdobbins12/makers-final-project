...
class TestKingInCheck:
        ...
        def test_king_not_in_check_from_own_pieces(self, run_before_tests):
                ...
                assert test_game.board[2][4].in_check(test_game.board, 2, 4) == False
                test_game.execute_turn(5,4,5,5)
                assert test_game.board[5][5].in_check(test_game.board, 5, 5) == False
                test_game.execute_turn(0,3,2,5) # black queen puts king in check
                assert test_game.board[5][5].in_check(test_game.board, 5, 5) == True # white king in check?
                assert test_game.board[2][4].in_check(test_game.board, 2, 4) == False # black king in check?
        ...
        def test_king_can_be_in_check(self, run_before_tests):
                ...
                assert test_game.board[7][4].in_check(test_game.board, 7, 4) == True
        ...
        def test_king_in_check_from_knight(self, run_before_tests):
                ...
                assert test_game.board[0][4].in_check(test_game.board, 0, 4) == True
