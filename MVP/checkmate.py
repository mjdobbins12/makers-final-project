...
class Checkmate:
        ...
        def __check_for_possible_king_moves(self):
                ...
                if king_in_question.in_check(self.game.board.board, king_coords[0], king_coords[1]) == True:
                    ...
                    for coords in possible_king_moves_array:
                        possible_move_target_square = self.game.board.board[coords[0]][coords[1]]
                        self.game.board.board[coords[0]][coords[1]] = king_in_question
                        self.game.board.board[king_coords[0]][king_coords[1]] = "-"
                        ...
                        self.game.board.board[king_coords[0]][king_coords[1]] = king_in_question
                        self.game.board.board[coords[0]][coords[1]] = possible_move_target_square
        ...
        def __check_for_possible_checkmate_blocks(self):
                ...
                if king_in_question.in_check(self.game.board.board, king_coords[0], king_coords[1]):
                # all_available_moves_for_all_same_colour_pieces:
                        for i in range(0,8):
                                for j in range(0,8):
                                    if isinstance(self.game.board.board[i][j], Piece) and not isinstance(self.game.board.board[i][j], King):
                                        if self.game.board.board[i][j].colour == king_in_question.colour:
                                            all_possible = self.game.board.board[i][j].available_moves(self.game.board.board, i, j)
                                            # print([all_possible, self.game.board.board[i][j], self.game.board.board[i][j].colour, i, j])
                                            for k in all_possible:
                                                original_piece = self.game.board.board[i][j]
                                                target_square = self.game.board.board[k[0]][k[1]]
                                                self.game.board.board[k[0]][k[1]] = self.game.board.board[i][j]
                                                self.game.board.board[i][j] = "-"
                                                if king_in_question.in_check(self.game.board.board, king_coords[0], king_coords[1]):
                                                    checkmate_evaluations.append(True)
                                                else:
                                                    checkmate_evaluations.append(False)
                                                self.game.board.board[i][j] = original_piece
                                                self.game.board.board[k[0]][k[1]] = target_square
                                            # print(checkmate_evaluations)
